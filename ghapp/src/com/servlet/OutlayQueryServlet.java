...
import com.form.OutlayQueryForm;
 ...
 *
 */
public class OutlayQueryServlet extends HttpServlet {

	private static final long serialVersionUID = -7197471783103272734L;

	public OutlayQueryServlet() {
		super();
	}
	public void destroy() {
		super.destroy(); 
	...
	protected void service(HttpServletRequest req, HttpServletResponse resq){
		...
		if(userid != null){
	        ...
	        if(LangUtils.isNotEmpty(postCode)) {
	            ...
	            if(postCode.indexOf("tai") != -1||postCode.indexOf("ghqthdsqcx") != -1||( null!= odUserForm && null!=odUserForm.getOdUser() && null!=odUserForm.getOdUser().getIsAdminRole() &&odUserForm.getOdUser().getIsAdminRole()==1)) {
...
//		String centercode = (String)req.getSession().getAttribute("centerCode");
		...
		/****全部****/
		OutlayDao oDao = new OutlayDao();
		List<WaitForm> allList = oDao.findallList(pagenum, "7", title, userid,parameterMap, null);
		
		/****待审批****/
		WaitDao wDao = new WaitDao();
		List<WaitForm> waitList = wDao.findWaitList(pagenum, userid, title, "7");
		
		/****审批中****/
		List<WaitForm> doingList = oDao.finddoingList(pagenum, "7", title, userid);
		
		/****待报销****/
		List<WaitForm> bxList = oDao.findallList(pagenum, "7", title, userid,parameterMap, "1");
